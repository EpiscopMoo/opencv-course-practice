...
int main(int argc, char* argv[])
{
  ...
  //DescriptorExtractor* descriptorExtractor = new DescriptorExtractor;
  ...
  vector<Point2f> homKeyptsSrc;
  vector<Point2f> homKeyptsDst;
  for (int matchInd = 0; matchInd < matches.size(); matchInd++)
  {
      homKeyptsSrc.push_back(keypoints1[matches[matchInd].queryIdx].pt);
      homKeyptsDst.push_back(keypoints2[matches[matchInd].trainIdx].pt);
      KeyPoint srcKeyPoint = keypoints1[matches[matchInd].queryIdx];
      KeyPoint scnKeyPoint = keypoints2[matches[matchInd].trainIdx];

      float x = srcKeyPoint.pt.x - center.x;
      float y = srcKeyPoint.pt.y - center.y;
      float distanceFromCenter = sqrt(x*x+y*y);
      float initialAngle = atan(y/x);

      float dAngle = toRad(scnKeyPoint.angle - srcKeyPoint.angle);
      float dScale = scnKeyPoint.size/srcKeyPoint.size;

      float scnDistanceFromCenter = dScale * distanceFromCenter;
      float finalAngle = dAngle + initialAngle;
      float newCenterX = (scnKeyPoint.pt.x + scnDistanceFromCenter * cos(finalAngle))/splitSet;
      float newCenterY = (scnKeyPoint.pt.y + scnDistanceFromCenter * sin(finalAngle))/splitSet;
      hgt.at<int>(newCenterY, newCenterX)++;
  }
  Point maxP;
  ...
  Mat homography = findHomography(homKeyptsSrc, homKeyptsDst, CV_RANSAC);
  Mat warped;
  warpPerspective(image, warped, homography, scene.size());
